# AWS Discovery Agent - Complete Implementation

# AWS Discovery Agent - Complete Implementation

**File**: `src/discovery/aws_[discovery.py](http://discovery.py)`

## Full Production Code (1,500+ lines)
...
class AWSDiscoveryAgent:
    ...
    def __init__(self, credentials: AWSCredentials):
        ...
        [self.ec](http://self.ec)2 = self.session.client('ec2', region_name=self.region)
        ...
        [logger.info](http://logger.info)(f"Initialized AWS Discovery Agent for region {self.region}")
    ...
    async def discover_all_resources(self) -> List[Resource]:
        ...
        """
        [logger.info](http://logger.info)(f"Starting full discovery in region {self.region}")
        ...
        [logger.info](http://logger.info)(f"Discovered {len(resources)} total resources")
        return resources
    ...
    async def _discover_ec2_instances(self) -> List[Resource]:
        ...
        try:
            paginator = [self.ec](http://self.ec)2.get_paginator('describe_instances')
            ...
            [logger.info](http://logger.info)(f"Discovered {len(resources)} EC2 instances")
    ...
    def _convert_ec2_to_resource(self, instance: dict) -> Resource:
        ...
        resource = Resource(
            provider=[CloudProvider.AWS](http://CloudProvider.AWS),
            resource_type=[ResourceType.EC](http://ResourceType.EC)2_INSTANCE,
    ...
    async def _discover_rds_instances(self) -> List[Resource]:
        ...
        try:
            ...
            [logger.info](http://logger.info)(f"Discovered {len(resources)} RDS instances")
    ...
    async def _discover_load_balancers(self) -> List[Resource]:
        ...
        try:
            ...
            [logger.info](http://logger.info)(f"Discovered {len(resources)} load balancers")
    ...
    async def _discover_lambda_functions(self) -> List[Resource]:
        ...
        try:
            ...
            [logger.info](http://logger.info)(f"Discovered {len(resources)} Lambda functions")
    ...
    async def _discover_s3_buckets(self) -> List[Resource]:
        ...
        try:
            ...
            [logger.info](http://logger.info)(f"Discovered {len(resources)} S3 buckets")
    ...
    async def _discover_ecs_services(self) -> List[Resource]:
        ...
        try:
            ...
            [logger.info](http://logger.info)(f\"Discovered {len(resources)} ECS services\")
    ...
    async def _discover_eks_clusters(self) -> List[Resource]:
        ...
        try:
            ...
            [logger.info](http://logger.info)(f\"Discovered {len(resources)} EKS clusters\")
    ...
    async def analyze_vpc_flows(
        ...
        \"\"\"
        [logger.info](http://logger.info)(f\"Analyzing VPC Flow Logs for {vpc_id}\")
        ...
        try:
            ...
            query_response = self.logs.start_query(
                logGroupName=[settings.AWS](http://settings.AWS)_FLOW_LOG_GROUP,
            ...
            for attempt in range(max_attempts):
                ...
                if status == 'Complete':
                    ...
                    [logger.info](http://logger.info)(f\"Found {len(flows)} unique data flows in {vpc_id}\")
                    return flows
                elif status == 'Failed':
    ...
    async def persist_resources(self, resources: List[Resource]) -> int:
        ...
        with get_db_context() as db:
            for resource in resources:
                ...
                if existing:
                    ...
                    [existing.security](http://existing.security)_groups = [resource.security](http://resource.security)_groups
                else:
                    # Insert new resource
                    db.add(resource)
                    count += 1
        ...
        [logger.info](http://logger.info)(f\"Persisted {count} new resources to database\")
        return count
...
async def run_aws_discovery(region: str = None) -> Dict[str, any]:
    ...
    Args:
        region: AWS region (defaults to [settings.AWS](http://settings.AWS)_REGION)
        
    Returns:
        Discovery statistics
    \"\"\"
    if region is None:
        region = [settings.AWS](http://settings.AWS)_REGION or 'us-east-1'
    
    credentials = AWSCredentials(
        access_key_id=[settings.AWS](http://settings.AWS)_ACCESS_KEY_ID,
        secret_access_key=[settings.AWS](http://settings.AWS)_SECRET_ACCESS_KEY.get_secret_value() if [settings.AWS](http://settings.AWS)_SECRET_ACCESS_KEY else None,
        region=region
    )
    ...
    # Discover resources
    resources = await [agent.discover](http://agent.discover)_all_resources()
...
✅ VPC Flow Log analysis via CloudWatch Logs Insights

✅ IP-to-resource mapping for flow correlation

✅ Tag-based scope detection (pci:scope, keywords)

✅ Automatic database persistence with upserts
...
✅ Cross-references: Uses models from `[[models.py](http://models.py) - Data Models](models%20py%20-%20Data%20Models%203b4aea15ac0c46889d4dd18246f7390b.md)`, config from `[[config.py](http://config.py) & [database.py](http://database.py) - Core Infrastructure](config%20py%20&%20database%20py%20-%20Core%20Infrastructure%206c0f96485fac4100a14c1bbc756cb2ad.md)`
