# Scope Classifier - Complete Implementation

# Scope Classifier - Complete Implementation

**File**: `src/analysis/scope_[classifier.py](http://classifier.py)`

## Full Production Code (1,000+ lines)
...
class ScopeClassifier:
    ...
    def __init__(self, db: Session):
        ...
        [logger.info](http://logger.info)(\"Initialized Scope Classifier\")
    ...
    def classify_resource(
        ...
        \"\"\"
        [logger.info](http://logger.info)(f\"Classifying resource {resource.resource_id}\")
    ...
    def _analyze_network_connections(self, resource: Resource) -> Optional[ClassificationResult]:
        ...
        outbound_flows = self.db.query(DataFlow).filter(
            DataFlow.source_resource_id == [resource.id](http://resource.id)
        ).all()
        ...
        for flow in outbound_flows + inbound_flows:
            ...
            other_resource_id = (
                flow.dest_resource_id if flow.source_resource_id == [resource.id](http://resource.id) 
                else flow.source_resource_id
            )
            
            # Check if other resource is CDE
            other_resource = self.db.query(Resource).filter(
                [Resource.id](http://Resource.id) == other_resource_id
            ).first()
    ...
    def _analyze_configuration(self, resource: Resource) -> Optional[ClassificationResult]:
        ...
        if [resource.security](http://resource.security)_groups:
            # Analyze security group rules
            restrictive_sg = self._analyze_security_groups([resource.security](http://resource.security)_groups)
            if restrictive_sg:
    ...
    def classify_all_pending(self) -> List[ScopeDecision]:
        ...
        \"\"\"
        [logger.info](http://logger.info)(\"Starting batch classification of pending resources\")
        
        # Get all resources needing classification
        pending_resources = self.db.query(Resource).join(
            ScopeDecision, 
            (ScopeDecision.resource_id == [Resource.id](http://Resource.id)) & ([ScopeDecision.is](http://ScopeDecision.is)_current == True)
        ).filter(
            ScopeDecision.scope == PCIScope.PENDING_REVIEW
        ).all()
        
        [logger.info](http://logger.info)(f\"Found {len(pending_resources)} resources pending classification\")
        ...
        self.db.commit()
        [logger.info](http://logger.info)(f\"Completed classification: {len(new_decisions)} decisions made\")
    ...
    def reclassify_resource(
        ...
        Returns:
            New scope decision
        \"\"\"
        [logger.info](http://logger.info)(f\"Reclassifying resource {resource.resource_id} (requested by {user_id})\")
        ...
        if current_decision:
            current_[decision.is](http://decision.is)_current = False
        ...
        if current_decision:
            decision.superseded_by_id = current_[decision.id](http://decision.id)
    ...
    def _create_scope_decision(
        ...
        decision = ScopeDecision(
            resource_id=[resource.id](http://resource.id),
            scope=result.scope,
            data_classification=[result.data](http://result.data)_classification,
    ...
    def get_scope_summary(self) -> Dict[str, int]:
        ...
        decisions = self.db.query(
            ScopeDecision.scope,
            func.count([ScopeDecision.id](http://ScopeDecision.id)).label('count')
        ).filter(
            [ScopeDecision.is](http://ScopeDecision.is)_current == True
    ...
    def validate_scope_dependencies(self) -> List[Dict]:
        ...
        cde_resources = self.db.query([Resource.id](http://Resource.id)).join(
            ScopeDecision,
            (ScopeDecision.resource_id == [Resource.id](http://Resource.id)) & ([ScopeDecision.is](http://ScopeDecision.is)_current == True)
        ).filter(
        ...
        flows = self.db.query(DataFlow).filter(
            (DataFlow.source_resource_[id.in](http://id.in)_(cde_ids)) |
            (DataFlow.dest_resource_[id.in](http://id.in)_(cde_ids))
        ).all()
        ...
        [logger.info](http://logger.info)(f\"Scope validation found {len(issues)} issues\")
        return issues
...
✅ CDE keyword detection (payment, card, tokenization, etc.)

✅ Network connectivity analysis (finds connected-to-CDE resources)

✅ Security configuration analysis

✅ Tag-based override support

✅ Confidence scoring for each classification

✅ Dependency tracking (which resources influenced decision)

✅ Batch classification for performance

✅ Scope validation (consistency checking)

✅ Automatic tag application

✅ Audit trail through ScopeDecision versioning
