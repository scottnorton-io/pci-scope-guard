...
class DataFlow(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    source_resource_id = Column(UUID(as_uuid=True), ForeignKey("resources.id", ondelete="CASCADE"), nullable=False, index=True)
    dest_resource_id = Column(UUID(as_uuid=True), ForeignKey("resources.id", ondelete="CASCADE"), nullable=False, index=True)
...
class ScopeDecision(Base):
    ...
    superseded_by_id = Column(UUID(as_uuid=True), ForeignKey("scope_decisions.id"), nullable=True)
    ...
    def __repr__(self):
        return f"<ScopeDecision scope={self.scope.value} current={self.is_current}>"
...
class Evidence(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    scope_decision_id = Column(UUID(as_uuid=True), ForeignKey("scope_decisions.id", ondelete="CASCADE"), nullable=False, index=True)
